...
public class AdminSettingsService
{
            ...
            if (setting is not null)
            {
                ...
                if (_log != null)
                    await _log.AuditAsync(
                        ...
                        $"Configura��o removida: {SiteIntermediaryPixKey}.",
                        actorUserId: actorUserId,
                        source: AdminAuditSources.AdminSettings);
            }
    ...
    public async Task<bool> SetReconciliationSeverityThresholdsAsync(int warningThreshold, int criticalThreshold, string? actorUserId = null)
    {
        ...
        if (_log is not null)
        {
            await _log.AuditAsync(
                ...
                $"Configura��o alterada: {ReconciliationWarningThresholdKey} = {normalizedWarning}.",
                actorUserId: actorUserId,
                source: AdminAuditSources.AdminSettings);
            ...
            await _log.AuditAsync(
                ...
                $"Configura��o alterada: {ReconciliationCriticalThresholdKey} = {normalizedCritical}.",
                actorUserId: actorUserId,
                source: AdminAuditSources.AdminSettings);
        }
    ...
    private static void EnsureAdmin(ClaimsPrincipal? user)
    {
        ...
        {
            throw new UnauthorizedAccessException("Apenas administradores podem alterar configura��es administrativas.");
        }
    }
}
