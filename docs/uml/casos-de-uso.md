...
|---|---|---|---|---|---|---|---|
| UC-J-01 | Registrar com e-mail/senha | `/Identity/Account/Register` | `RegisterModel.OnPostAsync` → `UserManager.CreateAsync` + role `user` + e-mail de confirmacao | E-mail nao cadastrado; senha conforme politica | Conta criada; `RegisterConfirmation` (ou auto-login se confirmacao nao exigida) | nao | `C33CriticalUseCaseTests.Register_Post_CreatesUserWithUserRole` |
...
| UC-J-07 | Trocar senha (logado) | `/Identity/Account/ChangePassword` | `ChangePasswordModel.OnPostAsync` (`UserManager.ChangePasswordAsync`, `RefreshSignInAsync`) | Autenticado; senha atual correta | Senha alterada + `UserPasswordChanged` | sim | `C33CriticalUseCaseTests.ChangePassword_Post_ChangesPassword` |
| UC-J-08 | Logout | `/Identity/Account/Logout` | `LogoutModel` (`SignInManager.SignOutAsync`) | Autenticado | Sessao encerrada | nao | `C33CriticalUseCaseTests.Logout_Post_Redirects` (GET+POST) |
| UC-J-09 | Entrar em grupo via convite | `/convite/{Code}` | `Join.razor.cs JoinGroup()` delega para `GroupDetailService.JoinWithCodeAsync` | Autenticado; `InviteCode` valido; nao-membro | `GroupMemberRole.Member` imediato (sem aprovacao) | sim (`GroupMemberAdded`, C34) | `GroupDetailServiceTests.JoinWithCodeAsync_*` (service cobre a pagina) |
| UC-J-10 | Solicitar entrada em grupo privado | `/grupo/{Id}`, `/futsal/{Id}`, `/poker/{Id}` | `GroupDetailService.RequestToJoinAsync`, `EventDetailService.RequestToJoinAsync` | Autenticado; grupo privado; sem request pendente | `GroupJoinRequest` Pending; admin notificado (badge) | sim (`GroupJoinRequested`, C34; ambos os services) | `GroupDetailServiceTests.RequestToJoinAsync_*`; `EventDetailServiceTests.RequestToJoinAsync_*`; `C34AuditTrailTests.RequestToJoin_WritesGroupJoinRequestedAudit` |
...
| UC-J-14 | Ver grupo, partidas, ranking | `/grupo/{Id}`, `/partidas`, `/ranking` | `GroupDetailService.LoadAsync`; EF direto | `/grupo` publico (capa); `/partidas` e `/ranking` exigem membro (C36-C-F2) | Visualizacao | nao | `GroupDetailServiceTests.LoadAsync_*`; `RankingWinCalculationTests`; guarda de membro coberta por `C36CGroupAccessTests` |
...
| UC-J-22 | Pix manual + enviar comprovante ** | `/pagamento/evento/{ConfirmationId}` | `EventPaymentService.GetGroupAdminPixKey`, `BuildPixStaticPayload`; `PixProofUploadService.UploadProofAsync` | Gateways off / `ShowDirectPixToOrganizer` | Dono da confirmacao (service revalida, C34-F3); <=5MB jpg/png/webp com magic bytes validos → `PixProofUploadedAt`; admin confirma manual | sim (`ProofUploaded`/`ProofReplaced`, C34) | `PixProofUploadTests` (14 casos); `PixManualPaymentFlowTests` (7 fluxos); `C34AuditTrailTests.UploadProof_*`; `C34SecurityAdversarialTests.UploadProofAsync_*` |
| UC-J-23 | Historico de pagamentos (filtros, export, cancelar) ** | `/payments`, `/payments/view/{PaymentId}` (fora da nav desde C36-C; viva p/ marketplace) | `PaymentsHistory.razor.cs`; `PaymentConfirmationService.ConfirmAsync` | Autenticado; dono (seller ve recebidos) | Lista/export CSV-PDF/cancela pendente/re-checa status | nao | `PaymentConfirmationServiceTests`; historico/export/cancel FALTA |
| UC-J-24 | Meus pagamentos no grupo + pendencias na home | `/grupo/{id}/pagamentos`, `/`, card da home | `GroupPaymentsService.LoadGroupAccessAsync`/`LoadMyPaymentsAsync`/`CountMyPendingAsync`/`PendingByGroup`; `MyPaymentsList`; `ConfirmationCard` | Membro (service revalida); so dados proprios | Lista A pagar/Em analise/Pago + total; banner por grupo; badge no hub | nao | `GroupPaymentsServiceTests` (access, my-payments, count); `C36CPendingPaymentTests` |
...
| UC-J-26 | Editar perfil (contatos, chave Pix) ** | `/profile/{Id}` proprio | `ProfileService.SaveOwnProfileAsync` (C34: extraido da page) → `UserManager.UpdateAsync` + audit | `isOwnProfile` | Contatos + `PixKey` salvos | sim (`UserProfileUpdated` so quando a PixKey muda, C34) | `ProfilePixEditableTests`; `ProfileCharacterizationTests`; `ProfilePixPrivacyTests`; `C34AuditTrailTests.SaveOwnProfile_*` |
...
| UC-O-13 | Marcar pagamento recebido ** | `/grupo/{id}/pagamentos` | `GroupPaymentsService.MarkPaidAsync` + `StampFeeOnPaidAsync` | Admin do grupo (service revalida + conf∈grupo, C34-F3) | `Paid` + `MarkedPaidBy` + taxa carimbada | sim | `GroupPaymentsServiceTests.MarkPaidAsync_*` |
| UC-O-14 | Toggle pago/pendente ** | `/futsal/{id}`, `/pagamento/evento/{id}` (admin view) | `AdminConfirmationService.TogglePaidAsync` (service revalida admin do grupo, C34-F3) | Admin do grupo | `PaymentStatus` toggled + fee | sim | `AdminConfirmationServiceTests.TogglePaidAsync_*` (7); `C34SecurityAdversarialTests.TogglePaidAsync_*` |
...
| UC-O-16 | Rejeitar comprovante ** | `/grupo/{id}/pagamentos` | `GroupPaymentsService.RejectProofAsync` | Admin do grupo (service revalida + conf∈grupo, C34-F3) | Proof limpo; segue nao-pago | sim | `C33CriticalUseCaseTests.RejectProofAsync_ClearsProof_KeepsPending_AndWritesAudit` |
| UC-O-17 | Aceitar comprovante → pago ** | `/grupo/{id}/pagamentos` | `GroupPaymentsService.MarkPaidAsync` | Admin do grupo (service revalida, C34-F3) + proof pendente | igual UC-O-13 | sim | `GroupPaymentsServiceTests.MarkPaidAsync_*`; `C34SecurityAdversarialTests.MarkPaidAsync_*` |
| UC-O-18 | Notificar inadimplente ** | `/grupo/{id}/pagamentos` | `GroupPaymentsService.NotifyDelinquencyAsync` → `EventNotificationService` | Admin do grupo (service revalida, C34-F3) | Mensagem interna/e-mail | sim | `DelinquencyNotificationTests` (14) |
...
| UC-O-20 | Enviar repasse com comprovante ** | `/grupo/{id}/pagamentos` | `PlatformFeeSettlementService.SubmitSettlementAsync` (re-verifica admin no service) | Admin; ≥1 partida pendente; valor=residual | `PlatformFeeSettlement` EmAnalise + Items | sim (`SettlementSubmitted`, C34) | `PlatformFeeSettlementServiceTests.SubmitSettlementAsync_*` (12); `C33CriticalUseCaseTests.SubmitSettlementAsync_*` (Postgres, 2); `C34AuditTrailTests.SubmitSettlement_WritesSettlementSubmittedAudit` |
...
| UC-A-22 | Confirmar repasse ** | `/admin/revenue` | `PlatformFeeSettlementService.ReviewSettlementAsync(approve)` | sysadmin; EmAnalise | `Pago` + ReviewedBy/At | sim (`SettlementConfirmed`, C34) | `PlatformFeeSettlementServiceTests.ReviewSettlementAsync_*`; `C33CriticalUseCaseTests.ReviewSettlementAsync_ApproveAndReject_OnPostgres`; `C34AuditTrailTests.ReviewSettlement_Approve_WritesSettlementConfirmedAudit` |
| UC-A-23 | Rejeitar repasse ** | `/admin/revenue` | `ReviewSettlementAsync(reject, note)` | sysadmin; motivo | `Rejeitado` + motivo | sim (`SettlementRejected` com motivo, C34) | `PlatformFeeSettlementServiceTests.ReviewSettlementAsync_Reject_*`; `C33CriticalUseCaseTests.ReviewSettlementAsync_ApproveAndReject_OnPostgres`; `C34AuditTrailTests.ReviewSettlement_Reject_WritesSettlementRejectedAuditWithReason` |
...
| GET | `/api/pix-proof/{id}` | RequireAuthorization; pagador ou admin do grupo ** | Serve imagem do comprovante | `C33CriticalUseCaseTests.PixProofEndpoint_*` (401/403/pagador/admin) |
| GET | `/api/fee-settlement-proof/{id}` | RequireAuthorization; submitter/admin grupo/sysadmin ** | Serve comprovante de repasse | `PlatformFeeSettlementProofAuthorizerTests`; `C33CriticalUseCaseTests.SettlementProofEndpoint_*` (401/403/submitter/sysadmin) |
