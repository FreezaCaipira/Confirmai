...
| UC-J-22 | Pix manual + enviar comprovante ** | `/pagamento/evento/{ConfirmationId}` | `EventPaymentService.GetGroupAdminPixKey`, `BuildPixStaticPayload`; `PixProofUploadService.UploadProofAsync` | Gateways off / `ShowDirectPixToOrganizer` | Upload <=5MB jpg/png/webp → `PixProofUploadedAt`; admin confirma manual | sim (`ProofUploaded`/`ProofReplaced`, C34) | `PixProofUploadTests` (14 casos); `PixManualPaymentFlowTests` (7 fluxos); `C34AuditTrailTests.UploadProof_*` |
...
| UC-O-16 | Rejeitar comprovante ** | `/grupo/{id}/pagamentos` | `GroupPaymentsService.RejectProofAsync` | Admin (page) | Proof limpo; segue nao-pago | sim | `C33CriticalUseCaseTests.RejectProofAsync_ClearsProof_KeepsPending_AndWritesAudit` |
