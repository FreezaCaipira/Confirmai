...
| i18n (regra 25) | **COMPLETO (PT/EN/ES)** -- ~880 strings migradas para chave (C25-C30C), teste anti-hardcode e allowlist zerada; os 3 dicionarios de produto (Group/Futsal/Poker) agora tem EN/ES completo e estao no `I18nKeyParityTests` | Nada. Manutencao natural. |
...
**Escopo cumprido nas 5 fases (0-4).** Fase 0: `Group.PlatformFeeWaivedFrom` + janela `[From, Until)`; `PlatformFeePolicy.ResolveStampForNewConfirmation` carimba a taxa **uma vez** na criacao da confirmacao (confirmacao direta e promocao da lista de espera, `EventDetailService`); QR (`EventPayment.ResolvedManualFee`), resumo (`EventPaymentSummary.ManualFee`), inadimplentes/comprovantes (`GroupPaymentsService`) e stamp legado leem o carimbo, e legados sem carimbo resolvem por `ConfirmedAt` (nao mais `UtcNow`). Resolve a ressalva 1 do C34. Fase 1: Pagamentos fora da nav, `/payments` vivo. Fase 2: `GroupAccess.IsMemberAsync/IsGroupAdminAsync/AdminGroupIdsAsync`; `/grupo/{id}/partidas` exige membro; `ApproveRequest/RejectRequest` revalidam admin pelo `request.GroupId`; `Admin*` de vaga/espera validam pelo `Event.GroupId` (fecha a ressalva 2 do C34). Fase 3: `LoadGroupAccessAsync` + `LoadMyPaymentsAsync` filtram por `UserId == currentUserId` **no service**, membro so ve o proprio; admin ganha aba "Meus pagamentos". Fase 4: banner de pendencias por grupo na home, badge no hub, CTA "Pagar" so para o proprio jogador. Build 0/0, 2511 testes, CI verde.

**Correcao 1 (dinheiro -- aplicada neste PR, nao bloqueia o merge ja feito)**: ao carimbar na criacao, `PlatformFeeAmount != null` deixou de significar "pago". Mas o ledger e o repasse ainda usavam isso como criterio: `GetGroupBalanceAsync` (`HasValue`), `GetGroupFeeBreakdownByMatchAsync`, `PlatformFeeSettlementQueryService` (visao do grupo e fila do admin) e `PlatformFeeSettlementService` (validacao do lote) somavam `PlatformFeeAmount > 0` **sem filtrar `PaymentStatus == Paid`**. Efeito: toda confirmacao nova de grupo futsal manual virava taxa devida pelo organizador no instante em que o jogador confirma, mesmo sem pagar (e continuava se o jogador nunca pagasse). Corrigido adicionando `PaymentStatus == Paid` nas 5 consultas + teste `Ledger_UnpaidStampedConfirmation_DoesNotAccrue`. Seed de `C33CriticalUseCaseTests` codificava o invariante antigo (Pending com carimbo) e foi ajustado. Licao para o Pleno: quando muda o **significado** de uma coluna, grep em todos os leitores dela, nao so nos que a tela usa.

**Correcao 2 (menor, aplicada)**: `SetWaiverAsync` sempre gravava `From = now`. Renovar uma isencao ativa (ex.: estender de 30 para 60 dias) movia `From` para frente e confirmacoes legadas sem carimbo criadas na vigencia anterior passavam a cair fora da janela. Agora renovacao de isencao ativa preserva `From`; audit registra o `From` efetivo. Teste `SetWaiver_RenewingActiveWaiver_KeepsOriginalFrom`.
...
### Fases adicionadas na execucao (review C33)
- **Fase 0** (commit `abf80c1`): ressalvas 2 e 3 do C33 -- `JoinWithCodeAsync` devolve `JoinWithCodeResult` (`Joined/AlreadyMember/InvalidCode`), sem texto PT no service; pages traduzem.
- **Fase 3b**: FALTA de permissao restantes -- `UC-O-24/26/27` via `EventCancellationService` (guarda unificada criador||sysadmin; **mudanca: poker passa a aceitar sysadmin e passa a auditar `event.cancelled`**, antes divergente do futsal) e `UC-A-08/09/10/12` via `AdminUserService` (audit com o **admin como ator** -- antes o log gravava o alvo; `UserRoleAssigned/Removed` existiam sem emissor). Convencao nova: toda `Pages/Admin/*.razor` exige `[Authorize(Roles="admin")]`.
...
**Leitura obrigatoria antes de comecar**: `docs/design-system.md` (tabela L0-L3, "filho nunca tem o tom do pai", cabecalho de painel nao e container, azul = direcionamento). O diagnostico do Robson para `/grupo/{id}`, `/mailbox` e `/profile` e o **mesmo** que ele deu para a home antes da #119: "as cores sao redundantes quanto aos niveis, fica tudo muito parecido e se torna uma coisa so, cansativo para os olhos". A causa tambem e a mesma: essas telas usam `--ci-bg-card`/`--ci-bg-card-deep`/`--ci-bg-alt` (alias para `--surface`/`--surface-2`) **sem regra de aninhamento**, mais gradientes legados (`event-detail.css .detail-card`, `Mailbox.razor.css` 5x, `Profile.razor.css` 1x) que achatam tudo. A #119 e o exemplo de referencia: ler o diff de `Pages/Index.razor.css` (`.home-section` L1 -> `.home-section-head` transparente + `border-bottom` -> `.home-event` L2 -> `:hover` L3) e repetir o mesmo raciocinio.

**Regra de aliases**: nao usar mais `--ci-*` em arquivo tocado -- trocar pelo token direto (`--surface`, `--surface-2`, `--border-2`, `--text-2`...). Motivo: `--ci-bg-mid` = `--surface-2`, `--ci-bg-deep` = `--surface`, `--ci-bg-alt` = `--surface` -- o nome nao diz o nivel e foi assim que o footer ficou no tom de card (achado da #119: `MainLayout.razor.css footer.oldsite-footer { background: var(--ci-bg-mid) }` vencia o `shell.css` por ser CSS isolado, carregado por ultimo). Ao tocar em qualquer seletor, `grep` o mesmo seletor em `site.css`, `entity-shell.css`, `event-detail.css`, `events.css`, `buttons.css` e no `.razor.css` da pagina -- **o ultimo carregado ganha** (`_Host.cshtml`: tokens -> site -> tables -> buttons -> entity-shell -> ... -> shell -> `Confirmai.styles.css`).

#### Fase 1 -- Botoes: um so sistema (ponto 1 do Robson: "botoes seguem o layout antigo, como o de grupos ou de voltar") -- EXECUTADO
Diagnostico: existem ~12 familias de botao nas telas do grupo (`detail-back-link`, `btn-view-groups`, `detail-admin-btn` + 8 modificadores por cor, `groups-create-btn`/`--semanais`, `invite-block-btn`, `invite-copy-btn`, `events-expand-btn`, `confirm-btn`, `cancel-join-btn`, `mk-btn`), todas em `event-detail.css`/`event-listing.css`/`buttons.css` (1858 linhas, 18 gradientes, 199 vars legadas) com gradiente, `translateY(-1px)` e sombra colorida. "Voltar" azul cheio e "Grupos" verde cheio competem com o CTA real da tela.
Fazer:
1. Criar `wwwroot/css/components.css` (carregar em `_Host.cshtml` **depois** de `shell.css`, antes de `Confirmai.styles.css`) com **4 variantes e nada mais**:
   ```css
   .btn            { min-height: 40px; padding: 0 var(--space-4); border-radius: var(--radius-sm); font-weight: var(--fw-medium); gap: var(--space-2); transition: background .15s, border-color .15s; }
   .btn-primary    { background: var(--accent); color: var(--accent-fg); border: 1px solid transparent; }          /* UM por tela */
   .btn-primary:hover { background: var(--accent-hover); }
   .btn-secondary  { background: var(--surface-2); color: var(--text); border: 1px solid var(--border-2); }       /* L2 */
   .btn-secondary:hover { background: var(--surface-3); border-color: var(--accent-border); }
   .btn-ghost      { background: transparent; color: var(--text-2); border: 1px solid transparent; }              /* Voltar, links de navegacao */
   .btn-ghost:hover { color: var(--accent-text); background: var(--accent-soft); }
   .btn-danger     { background: transparent; color: var(--danger); border: 1px solid var(--danger); }           /* destrutivo, confirmado por modal */
   .btn-sm         { min-height: 32px; padding: 0 var(--space-3); font-size: 0.8125rem; }
   ```
   `.home-btn`/`.home-btn--primary` (Index.razor.css) e `.mk-btn` (buttons.css) viram **alias** dessas classes (regra no proprio components.css: `.home-btn, .mk-btn { @extend nao existe -> copiar as mesmas declaracoes }`) ate serem trocados no markup; nao apagar `.mk-btn` (Profile usa).
2. Mapear nas telas do grupo (`Groups/Detail|Partidas|Index|Payments|Ranking|Features.razor`, `Shared/Components/Groups/*.razor`):
   - "Voltar" (`detail-back-link`) -> `btn btn-ghost` com `fa-arrow-left`, **alinhado a esquerda do cabecalho do painel, fora do centro**.
   - "Grupos" verde (`btn-view-groups`) em `Partidas.razor` -> **remover**: o breadcrumb `Grupo > Partidas` ja faz isso e a nav tem "Grupos".
   - "Nova partida" (`groups-create-btn`) -> `btn btn-primary` (e o unico azul cheio de `/partidas` para o admin). "Semanais" verde -> `btn btn-secondary`.
   - `detail-admin-btn--partidas|--pagamentos|--ranking|--whatsapp|--settings` (hub) -> todos `btn btn-secondary` com icone; **cor nao diferencia funcao**, o rotulo diferencia. Excecao: "Convidar" (WhatsApp) no hub e o proximo passo de um grupo com 1 membro -> `btn-primary` **so** enquanto `Members.Count == 1`; depois vira secundario e o primario passa a ser "Nova partida" (admin) / "Confirmar presenca" na proxima partida (jogador). Estado derivado dos dados ja carregados em `Detail.razor.cs`, sem query nova.
   - "Copiar" (`invite-copy-btn`) -> `btn btn-ghost btn-sm`. "Acessar" na tabela de partidas -> `btn btn-secondary btn-sm`. "Entrar" com codigo em `/grupos` -> `btn-secondary` (a acao primaria da tela e "Criar novo grupo", ver Fase 4).
3. Apagar as regras substituidas em `event-detail.css`/`event-listing.css` (nao deixar CSS morto) e reportar linhas antes/depois. Meta em `buttons.css`: nenhuma regra nova; remocoes bem-vindas se `grep -rn "classe" --include=*.razor --include=*.cshtml` der zero.
Teste: novo `CssComponentsTests` -- `components.css` carregado em `_Host.cshtml` apos `shell.css`; nenhum `.razor` em `Pages/Groups/` usa `detail-back-link`, `btn-view-groups`, `groups-create-btn`, `detail-admin-btn--*`.

**Resultado**: `wwwroot/css/components.css` (131 linhas) com `.btn` + 4 variantes (`primary`/`secondary`/`ghost`/`danger`) + `.btn-sm`; aliases temporarios `.home-btn`/`.mk-btn` copiam as mesmas declaracoes (sem `@extend`). Registrado em `_Host.cshtml` depois de `shell.css`, antes de `Confirmai.styles.css`. Markup migrado em `Pages/Groups/` (Detail/Partidas/Features/Ranking/Payments/Index/Join) + `Shared/Components/Groups/` (GroupDetailEvents, NoGroupsHint): Voltar -> `btn-ghost` com `fa-arrow-left`; Copiar -> `btn-ghost btn-sm`; Convidar WhatsApp -> `btn-primary` so enquanto `Members.Count == 1` (depois secundario); nav do hub -> `btn-secondary`; "Nova partida" -> `btn-primary`, "Semanais" -> `btn-secondary`; "Acessar" na tabela -> `btn-secondary btn-sm`; "Entrar" com codigo -> `btn-secondary`, "Criar novo grupo" -> `btn-primary`; back-links nus dos estados `detail-not-found` -> `btn-ghost`. CSS morto removido (6 familias sem nenhum uso restante): `btn-view-groups`, `groups-create-btn`(+`--semanais`), `invite-copy-btn`, `invite-block-btn`, `events-expand-btn`, `event-access-btn`. `detail-back-link`/`detail-admin-btn`/`confirm-btn`/`cancel-join-btn`/`mk-btn` **mantidos** -- Futsal/Poker/Payment ainda usam (fora do escopo da fase); `groups-view-events-btn` e `group-card-approve-btn` ficam para a Fase 4 por decisao do plano. Botoes internos das abas de gestao de pagamentos (`payments-tab`, `proof-review-*`, `platform-fee-*`) nao estavam na lista da fase e ficam para quando a tela for elevada. Camadas: telas migradas mantem a estrutura atual (L0 fundo -> painel L1 -> elementos L2); a reestruturacao em paineis independentes e a Fase 7.

**Metricas CSS (arquivos tocados, antes->depois)**: linhas 3891->3659 (-232); `linear-gradient` 34->30 (-4); vars legadas `--futsal|poker|parchment|gold|green|red-*` 150->124 (-26); aliases `--ci-*` 283->258 (-25). `components.css`: 0 gradientes, 0 vars legadas. O restante pertence a Futsal/Poker/Payment (fases futuras).

**Testes**: `CssComponentsTests` (4): ordem de carga em `_Host`, 4 variantes + `.btn-sm` definidas, sem gradiente/var legada no arquivo, zero familia antiga em `Pages/Groups/`+`Shared/Components/Groups/`. Suite: 2513 -> 2517. Build `--no-incremental` 0/0.

#### Fase 2 -- Hover na data da partida (ponto 2: `/grupo/{id}/partidas`) -- EXECUTADO
Hoje `GroupDetailEvents.razor` linhas 62 e 111 mostram `dd/MM` + `HH:mm` (`UiTextService.FormatDateTime` "DateDefault"/"TimeDefault"). Nenhum dia da semana em lugar nenhum da tabela.
Fazer:
1. Envolver data+hora em `<span class="et-when" tabindex="0" aria-describedby="et-tip-@ev.Id">` e um `<span role="tooltip" id="et-tip-@ev.Id" class="et-tooltip">` com **3 linhas**: (a) dia da semana + data completa por idioma -- novo formato `"DateLong"` em `UiTextService.FormatDateTime` (PT `dddd, dd 'de' MMMM`, EN `dddd, MMMM dd`, ES `dddd, dd 'de' MMMM`; capitalizar a 1a letra); (b) relativo: "hoje" / "amanha" / "em N dias" / "ha N dias" (chaves `Event.RelativeToday|Tomorrow|InDays|DaysAgo` nos 3 idiomas; calculo em metodo estatico `EventDateText.Relative(DateTime startsAt, DateTime now)` em `Services/Events/`, com teste unitario nas 4 faixas); (c) local (`ev.Venue?.Name ?? ev.Location`) se existir, e "Semanal" se `RachaScheduleId.HasValue`.
2. Tooltip **CSS puro** (sem JS): `.et-when { position: relative }`, `.et-tooltip { position:absolute; top:100%; left:0; z-index:20; display:none; background: var(--surface-3); border:1px solid var(--border-2); border-radius: var(--radius-sm); padding: var(--space-2) var(--space-3); box-shadow: var(--shadow-card); white-space: nowrap; font-size: .8125rem; color: var(--text) }`, `.et-when:hover .et-tooltip, .et-when:focus-within .et-tooltip { display:block }`. E L3 (popover) -- ver design-system §1. Mobile (<=768px): tooltip vira linha visivel abaixo da data (`display:block; position:static`), porque nao existe hover em touch.
3. Aproveitar e colocar **o dia da semana abreviado sempre visivel** antes da data (`seg 23/09`) -- custa uma chave de formato (`"DateWeekday"` = `ddd dd/MM`) e resolve 80% do que o hover resolve. O hover fica para o detalhe.
Teste: `UiTextServiceStaticTests` ganha `DateLong`/`DateWeekday` nos 3 idiomas; `EventDateTextTests` (hoje/amanha/em 3 dias/ha 2 dias); teste de integracao (WebApplicationFactory, padrao `HomeGroupFirstIntegrationTests`) verificando que `/grupo/{id}/partidas` renderiza `role="tooltip"` com o dia da semana.

**Resultado**: `FormatDateTime` ganhou `DateLong` (PT/ES `dddd, dd 'de' MMMM`, EN `dddd, MMMM dd`, 1a letra capitalizada) e `DateWeekday` (`ddd dd/MM`) -- ambos cultura-aware como `DateTimeFullLong`. `EventDateText.Relative` (estatico, `Services/Events/`) devolve `EventRelativeDate(Kind, Days)` com comparacao por **dia de calendario** (nao 24h) -- o componente mapeia para `Event.RelativeToday|Tomorrow|InDays|DaysAgo` x3. `GroupDetailEvents` (web + mobile): data+hora dentro de `.et-when` (`tabindex=0`, `aria-describedby`) com `role="tooltip"` `.et-tooltip` de 3 linhas (DateLong / relativo em `--accent-text` / venue + `Group.WeeklyTitle` quando semanal); ids unicos por celula (`et-tip-N` / `et-tip-m-N` -- web e mobile coexistem no DOM). CSS puro `:hover`/`:focus-within`; em <=768px o tooltip vira linha estatica abaixo da data. `et-date` agora mostra `DateWeekday` ("seg. 22/06") -- dia da semana sempre visivel. **Correcao de bug aproveitada**: a tabela formatava `ev.StartsAt` cru (UTC) enquanto o resto do app chama `.ToLocalTime()` -- agora converte. `Partidas.razor.cs` ganhou `Include(e => e.Venue)` (nome do local no tooltip). Testes: `UiTextServiceStaticTests` +6 (DateLong x3, DateWeekday x3), `EventDateTextTests` +5 (4 faixas + borda de calendario), `PartidasDateTooltipIntegrationTests` +1 (`role="tooltip"`, `aria-describedby`, `et-date` com dia da semana, venue). Suite: 2537 -> 2549. Camadas: L0 fundo -> painel L1 -> linhas `.events-table-row` L2 -> `.et-tooltip` L3.

#### Fase 3 -- Hero "Minhas partidas" em azul (ponto 3: "todos os cards sao escuros, que tal fazer o contrario com ele?") -- EXECUTADO
Decisao do Senior: **aceito como a unica excecao a regra "sem gradiente"**, porque o hero e L1 sem filhos-card (so texto + toggle) e e o unico elemento da tela cujo papel e orientar. Registrar a excecao em `docs/design-system.md` §1 regra 5 ("Sem gradiente de fundo. Excecao unica: o hero de pagina, que pode usar o degrade de acento para se distinguir dos paineis de conteudo").
Fazer em `Pages/Index.razor.css` `.home-hero` (e `.home-hero--landing`):
```css
.home-hero { background: linear-gradient(135deg, var(--accent-hover) 0%, var(--accent) 55%, #3b82f6 100%); border: 1px solid var(--accent-border); color: var(--accent-fg); }
.home-hero .home-hero-title { color: var(--accent-fg); }
.home-hero .home-hero-sub   { color: rgba(255,255,255,.82); }   /* -> criar token --accent-fg-2 em tokens.css, nao rgba solto */
```
- O `#3b82f6` **nao pode ficar no arquivo**: criar `--accent-light: #3b82f6` em `tokens.css` (ja existe como `--ci-accent-opacity-*` base). Criar tambem `--accent-fg-2: rgba(255,255,255,0.82)` para o subtitulo.
- Toggle Jogo/Organizo dentro do hero: fundo `rgba(255,255,255,.14)` -> token `--on-accent-soft`; item ativo `--accent-fg` fundo + `--accent` texto (inverte). Contador do toggle igual.
- Contraste: `DesignTokensContrastTests` ganha os pares `accent-fg`/`accent` (>= 4.5 -- ja passa, e o CTA) e `accent-fg-2`/`accent` (>= 4.5; se falhar, subir a opacidade para .88). Testar o texto **sobre o ponto mais claro** do degrade (`--accent-light`): `text`/`accent-light` >= 4.5 -- se reprovar, terminar o degrade em `--accent`, nao em `--accent-light`.
- Landing deslogada (`.home-hero--landing`) segue igual: e onde o degrade mais faz sentido (marca + CTA "Criar conta" **branco** sobre azul, "Entrar" ghost branco).
Print antes/depois 1366 e 390; se ficar "banner de propaganda", reduzir para degrade sutil (`--accent` -> `--accent-hover`, 10% de variacao) -- o Robson decide no PR entre as duas variantes.

**Resultado**: `.home-hero` ganhou o degrade de acento `135deg var(--accent-hover) -> var(--accent)` + `color: --accent-fg` (inset-shadow de acento removido). **Desvio documentado do desenho**: o gradiente termina em `--accent`, nao em `--accent-light` (#3b82f6) -- texto sobre o ponto mais claro reprovaria AA (`text`/`accent-light` ~= 3.4:1 < 4.5), e a propria spec mandava esse fallback. `--accent-light` nao foi criado (token ficaria sem uso). Tokens novos em `tokens.css`: `--accent-fg-2: #eef4ff` (hex solido ~= rgba .92; passa 4.68:1 sobre `--accent` -- a spec sugeria rgba .82, que daria ~= 4.0:1 e falharia) e `--on-accent-soft: rgba(255,255,255,.14)` (trilha do toggle). Hero: titulo `--accent-fg`, sub `--accent-fg-2`. Toggle Jogo/Organizo dentro do hero: trilha `--on-accent-soft`, itens `--accent-fg-2`, ativo inverte (`--accent-fg` fundo + `--accent` texto), contador acompanha. CTAs da landing dentro do hero: `home-btn--primary` inverte (branco `--accent-fg` fundo + `--accent` texto = "Criar conta" branco sobre azul), `home-btn` vira ghost branco (borda rgba .45, hover `--on-accent-soft`). `docs/design-system.md` §1 regra 5 registra a excecao unica. `DesignTokensContrastTests` +5 pares: `accent-fg-2`/`accent`, `accent-fg`/`accent-hover`, `accent-fg-2`/`accent-hover`, `text`/`accent`, `text`/`accent-hover` -- todos >= 4.5 (o degrade inteiro e AA). `CssNoLegacyVarsTests` continua verde: `Index.razor.css` esta em `MigratedFiles` e a regex de excecao so cobre regras `.home-hero*`. Screenshots 1366/390 ficam para a validacao manual do Robson (E2E nunca executado; ele compara as duas variantes no PR -- a sutil foi a unica que passou AA). Suite: 2554 -> 2559. Camadas: L0 fundo -> hero L1 de acento (orienta) -> toggle/CTAs L2 sobre ele -> paineis de conteudo L1 normais abaixo.

#### Fase 4 -- `/grupos` vs home (ponto 4: opcao 1 escolhida pelo Robson; Senior concorda) -- EXECUTADO
Papel de cada tela, a partir de agora (documentar em `docs/design-system.md` §6 "Mapa de telas"):
- `/` = **o que acontece**: proximas partidas dos meus grupos, historico, pendencias. Chips de grupo sao **atalhos** para o hub, nada mais.
- `/grupos` = **gestao de participacao**: entrar em grupo (codigo), criar grupo, sair de grupo, pedidos de entrada pendentes (meus e, para admin, dos outros), convite. Nao lista partidas.
Fazer em `Pages/Groups/Index.razor(.css)` + `wwwroot/css/event-listing.css` (`.group-card*`, `.groups-*`):
1. Card de grupo vira **linha de gestao** (L2 dentro do painel L1 "Meus grupos"): icone do esporte em `--accent-soft`, nome, cidade, `N membros`, badge de papel (`Admin` em `--accent-soft`), badge de pendencia (`Pix pendente` -> `--warning-soft`; `Pedido em analise` -> `--accent-soft`), e a direita `btn-secondary btn-sm` "Abrir" + menu `...` com "Copiar convite" (admin) / "Sair do grupo" (`btn-danger`, confirm). **Sem** fundo verde por esporte, sem borda vermelha/azul, sem "proxima partida" (isso e a home).
2. Painel "Entrar em outro grupo" -> reutilizar `Shared/Components/Groups/GroupEntryPanel.razor` (C36-A) com parametro `Compact="true"`: "Criar novo grupo" = `btn-primary` (o proximo passo de quem ja tem grupo), codigo + "Entrar" = `btn-secondary`. Apagar o CSS proprio de `Index.razor.css` (117 linhas -> meta < 60). Botao verde "Ver Partidas" **sai**.
3. Pedidos pendentes de entrada (quando o usuario pediu e o admin ainda nao aprovou -- `GroupJoinRequest`): hoje nao aparecem em lugar nenhum para o solicitante; mostrar como linha L2 com badge `--accent-soft` "Aguardando aprovacao" e `btn-ghost btn-sm` "Cancelar pedido" (reusar `cancel-join-btn` logica de `Detail.razor`). Se a query nao existir em `GroupsService`, criar `GetMyPendingRequestsAsync(userId)` com teste.
4. Titulo do painel: `Meus grupos (N)` com filete azul e contador -- **mesma classe** `.home-section-head`/`.home-view-count` da home; mover essas classes de `Index.razor.css` para `components.css` (`.panel`, `.panel-head`, `.panel-title`, `.panel-count`, `.panel-body`) e a home passa a usa-las tambem. Zero mudanca visual na home.
Teste: `GroupsIndexIntegrationTests` (padrao `HomeGroupFirstIntegrationTests`): usuario com 2 grupos ve 2 linhas e "Criar novo grupo" como primario; usuario com pedido pendente ve "Aguardando aprovacao"; string "Ver Partidas" nao aparece.

**Resultado**: `/grupos` virou gestao de participacao. Linha de gestao `.group-row` (L2) com badge de esporte `--accent-soft`, nome, cidade, `N membros`, badge de papel (`Admin`/`Membro` em `status-badge--accent`/`status-badge`), badge de pendencias do admin (`N pendente(s)` `status-badge--accent` com borda `--accent-border`), `btn-secondary btn-sm` "Abrir" + menu `...` (`group-menu`, popover L3): "Copiar convite" (admin) e "Sair do grupo" (`btn-danger` com confirmacao inline -- `GroupDetailService.LeaveGroupAsync` novo retornando `LeaveGroupResult` (`NotMember`/`SoleAdmin`/`Left`), auditoria via `AuditEvents.GroupMemberRemoved` + `AuditEntities.GroupMember` com `leftVoluntarily`; **admin unico nao sai** (precisa promover outro antes -- erro inline `group-leave-error`)). "Ver Partidas" e o fundo verde por esporte morreram. Pedidos pendentes do proprio usuario: `GetMyPendingRequestsAsync` (query nova -- nao existia em lugar nenhum) renderiza `.request-row` com "Aguardando aprovacao" + `btn-ghost btn-sm` "Cancelar pedido" (`CancelJoinRequestAsync` agora exige `currentUserId` e revalida ownership -- caller do hub atualizado). `GroupEntryPanel` ganhou `Compact` (header sao e o painel L1 externo assume) e migrou para `btn` variants. Titulo "Meus grupos" com `.panel-count` (`home-section-count` movido para `components.css` -- home usa a mesma classe, zero mudanca visual). `Index.razor.css` 117->157 linhas (sistema de linhas + menu), `event-listing.css` 322->76 (todo o `.group-card*`/`.groups-*` morto saiu), `events.css` 1551->1218 (mesmos blocos + media queries orfas; `.join-code-input:focus`, `.groups-action-row`, `.group-card-approve-btn` removidos como ultimos restos). Ordenacao admin-com-pendencias primeiro preservada. Testes: `GroupsIndexIntegrationTests` (36 -- linhas, Abrir, primario, badges, contagens, estados de pedido, filtro, scoped-attr tolerante) + `GroupsIntegrationTests` atualizado para o markup novo + `GroupDetailServiceTests` (+4: pending requests, cancel com ownership, leave com audit, admin bloqueado). Chaves i18n novas x3 (Sair do grupo, Copiar convite, Aguardando aprovacao, Cancelar pedido, Entrar em outro grupo...). Camadas: L0 fundo -> painel L1 "Meus grupos" -> linhas `.group-row`/`.request-row` L2 -> menu `...`/confirmacao inline L3.

#### Fase 5 -- `/mailbox` (ponto 5: "totalmente ilegivel, versao antiga do layout") -- EXECUTADO
`Pages/Mailbox.razor.css`: 721 linhas, 5 gradientes, **100** vars legadas -- e a tela mais atrasada. Titulo "CAIXA POSTAL" em azul sobre azul (ilegivel: `--ci-accent` sobre `--ci-accent-mid`), 4 caixas aninhadas no mesmo tom.
Fazer (so CSS + classes; **nenhuma** mudanca em `Mailbox.razor.cs`):
1. Estrutura em 3 niveis, e so 3: painel L1 (`.mailbox-panel`, `--surface`, `--radius-lg`) com cabecalho `.panel-head` (transparente + `border-bottom`, titulo "Caixa postal" em `--text` + filete azul, contador de nao lidas em `.panel-count`) -> corpo em **2 colunas** `grid-template-columns: 320px 1fr` (>= 980px; 1 coluna abaixo) -> esquerda: filtros compactos numa linha (busca com icone, select, checkbox) e a lista de conversas como linhas L2 (`--surface-2`, `--border-2`; nao lida = `border-left: 3px solid var(--accent)` + `--fw-semibold`; selecionada = L3 `--surface-3` + `--accent-border`); direita: painel de leitura L2 com cabecalho proprio (remetente, data com `DateLong` da Fase 2) e mensagens como bolhas -- minhas em `--accent-soft`/`--accent-text`, dos outros em `--surface-3`.
2. Abas "Conversas (N) / Arquivadas (N)" -> mesmo toggle da home (`.home-view-toggle` -> mover para `components.css` como `.seg-toggle`, `.seg-toggle-item`, `.seg-toggle-item.active`).
3. Botoes: "Aplicar filtros" `btn-secondary btn-sm`, "Limpar" `btn-ghost btn-sm`, "Enviar" (responder) `btn-primary` (o unico azul cheio), "Arquivar"/"Marcar lida" `btn-ghost btn-sm`.
4. Empty states: "Nenhuma conversa" e "Selecione uma conversa" em L2 com `border: 1px dashed var(--accent-border)` (a mesma classe `.home-empty` -> `components.css` `.empty-state`).
5. Zerar gradientes e `--ci-*` no arquivo; meta < 350 linhas.
Teste: `CssNoLegacyVarsTests` (novo, generico): para cada arquivo em uma lista `MigratedFiles` (`Index.razor.css`, `Mailbox.razor.css`, `Profile.razor.css`, `Groups/*.razor.css`, `components.css`, `shell.css`, `tokens.css`) falha se encontrar `linear-gradient` (exceto `.home-hero`) ou `var(--ci-|--futsal-|--poker-|--green-|--red-|--parchment-|--gold-`. A lista **so cresce**.

**Resultado**: tela reescrita em 3 niveis -- painel L1 (`.panel .mailbox-panel`, head com `panel-title` + `panel-count` de nao lidas) -> corpo `grid 320px/1fr` (>=980px; 1 coluna abaixo) -> esquerda: `MailboxFilters` em linha compacta (busca com icone, select, checkbox; "Aplicar" `btn-secondary btn-sm`, "Limpar" `btn-ghost btn-sm`) + lista `.mailbox-list-item` L2 (nao lida = `border-left: 3px var(--accent)` + semibold; selecionada = L3 `--surface-3` + `--accent-border`; acoes Arquivar/Marcar lida `btn-ghost btn-sm`); direita: `MailboxThreadPane` L2 com head proprio (remetente + `DateLong` da ultima mensagem + N mensagens) e bolhas (incoming `--surface-3`, outgoing `--accent-soft`/`--accent-text`) + composer com unico `btn-primary` da tela ("Enviar"). Abas Conversas/Arquivadas viraram `.seg-toggle` com `.seg-count`. Empty states em `.empty-state` -- a classe ganhou o `border: 1px dashed var(--accent-border)` + `--surface-2` do `.home-empty` (harmonizacao; todas as telas que ja usam `.empty-state` ganham o mesmo visual). `.input` novo em `components.css` (campo L2, foco `--accent-ring`) -- `mk-input` era scoped a `.marketplace-page` e **nem chegava a estilizar** esses campos. Mortos: `mailbox-shell-card`, header "CAIXA POSTAL" com border-image + text-shadow, `mailbox-workspace`, `mailbox-chip`, `mailbox-list-item-peer` ("Conversa em andamento" repetido em toda linha), `mailbox-thread-bubble-{role}` (RoleClass nunca era setado -- propriedade morta), `market-empty`, `mk-btn` x8. `Mailbox.razor.cs` intacto conforme a spec. **Bonus**: resíduos `--ci-*`/`--poker-*` zerados em `Ranking.razor.css`, `NoGroupsHint.razor.css`, `RankingTable.razor.css`, `RankingViewSelector.razor.css` (11 ocorrencias -> tokens). Metricas: mailbox css 1355 -> 398 linhas (-957), gradientes 7 -> 0, vars legadas 199 -> 0; `Pages/Create` e `Payments.razor.css`/`GroupDetailPaymentsModal` seguem fora (gestao de pagamentos do grupo nao e escopo do C36-D). `CssNoLegacyVarsTests` novo: `MigratedFiles` (25 arquivos) falha em `linear-gradient` fora de `.home-hero` e em qualquer familia legada -- a lista so cresce. Suite: 2549 -> 2552. Camadas: L0 fundo -> painel L1 -> linhas/bolhas L2 -> selecionada/foco L3.

#### Fase 6 -- `/profile/{id}` (ponto 6: mesmo tratamento do mailbox) -- EXECUTADO
`Pages/Profile.razor.css` (182 linhas, 1 gradiente, 23 legadas) + `Pages/Components/Profile/*.razor.css` + `.mk-btn` em `buttons.css`.
Fazer:
1. Cabecalho do perfil: hoje gradiente azul com avatar -- passa a **L1 sem gradiente** (`--surface`, borda `--border`): avatar 64px com anel `2px solid var(--accent)`, nome em `--text` `font-size: 1.25rem` (criar `--fs-xl` em tokens.css), e-mail/desde em `--text-2`. (O degrade e exclusivo do hero da home, Fase 3.)
2. Blocos "Meus dados", "Recebimento (Pix)", estatisticas por esporte -> paineis L1 com `.panel-head` (filete azul + titulo em `--text`, **sem** caixa-alta forcada -- `text-transform` sai); campos `input` em L2 (`--surface-2`, borda `--border-2`, foco `--accent-ring`); "Salvar" = `btn-primary` **um por painel** (cada painel e um formulario independente -- aqui a regra "um azul por tela" vale **por formulario**, registrar no design-system §2).
3. Abas Futsal/Poker (gradiente azul, ilegivel) -> `.seg-toggle` (Fase 5). Cards de estatistica (Partidas / Linha / Goleiro) -> L2 com numero em `--text` `--fs-2xl` (criar em tokens.css: 1.5rem; junto `--fs-xl` 1.25rem) e rotulo `--text-2` sem caixa-alta; **numero em azul so quando > 0 e e o destaque** -- na duvida, `--text`.
4. Chave Pix nao configurada: e pre-requisito de receber -> `empty-state` com CTA "Cadastrar chave Pix" (`btn-primary` do painel), nao texto cinza.
Teste: entra na lista `MigratedFiles`; integracao: HTML de `/profile` nao contem `mk-btn` se todos os usos migrarem (senao manter alias).

**Resultado**: a pagina virou uma coluna de paineis L1 (`.panel`), `.entity-shell` inerte removido. (a) `ProfileHeaderCard`: L1 **sem gradiente** -- avatar 64px com anel `2px solid var(--accent)` (iniciais em `--accent-soft`/`--accent-text`, sem degrade), nome `--fs-xl`, "membro desde" `--text-2` sem caixa-alta nem text-shadow; overlay de troca de avatar com `focus-visible`; salvar avatar `btn-primary`. (b) `ProfileEditForm` "Meus dados": `.panel-head` com `panel-title` (sem uppercase/border-image); campos `InputText class="input"` -- o `.input` novo de `components.css` (criado na Fase 5) ja cobre L2 + foco `--accent-ring`; Salvar `btn-primary` do painel. (c) Pix: `panel` com panel-head; **sem chave** -> `.empty-state` (icone + hint + input + `btn-primary` "Cadastrar chave Pix" -- chave nova `Profile.PixRegister` x3); **com chave** -> linha label+input+"Salvar" `btn-primary`; highlight `?pix` mantido com `--accent-ring`. (d) `ProfileSportStats`: tabs viraram `.seg-toggle`/`seg-item` (fora do gradiente azul ilegivel); stat cards L2, numero `--fs-2xl` `--text`, `--accent-text` **so no total > 0** (destaque do painel). (e) Chat (perfil alheio): `panel` + panel-head; bolhas incoming `--surface-3` / outgoing `--accent-soft`; composer `.input` + Enviar `btn-primary` + Atualizar `btn-ghost btn-sm`; empty-state `.empty-state`. (f) `ProfileContactsDisplay`: `panel` com linhas L2, labels `--text-3` sem uppercase. **Componente morto apagado**: `AvatarUploadSection` (x3 arquivos) -- nao era referenciado por nenhuma pagina (o header faz upload inline). `mk-btn`/`mk-input` zerados no fluxo. Metricas: profile css 755 -> 394 linhas (-361), gradientes 3 -> 0, vars legadas 70 -> 0 (ficaram `--ci-*` so em `AvatarUploadSection` -- apagado). `MigratedFiles` 25 -> 32. Testes: `ProfilePageIntegrationTests` (2: sem mk-*, panels/seg-toggle/btn-primary; empty-state Pix com CTA). Suite: 2552 -> 2554. Camadas: L0 fundo -> paineis L1 -> campos/linhas/stats/bolhas L2 -> selecionada/foco/highlight L3.

#### Fase 7 -- Hub `/grupo/{id}` com elevacao (ponto 7: "cores redundantes quanto aos niveis; melhorar a visibilidade dos botoes") -- EXECUTADO
Arquivos: `Pages/Groups/Detail.razor(.css)` (340 linhas, 2 gradientes, 26 legadas), `wwwroot/css/event-detail.css` (`.detail-card`, `.detail-section*`, `.detail-admin-btn*`, `.invite-block*`), `Shared/Components/Groups/GroupMetrics.razor.css`, `Pages/Groups/Components/MembersManager.razor.css` (16 legadas).
Diagnostico (print de 14/06): `.detail-card` e um cartao unico de 680px com gradiente `--ci-bg-alt -> --ci-bg` contendo bloco de convite (mesmo tom), 4 metricas (mesmo tom, icones com gradiente colorido), lista de membros (mesmo tom) -- 4 niveis no mesmo cinza; "Voltar" azul cheio e "Convidar" verde cheio no topo brigam; botoes "Partidas" (azul) e "Pagamentos" (verde) escondidos no cabecalho de Membros.
Fazer:
1. **Matar o `.detail-card` unico**: a pagina passa a ser uma coluna de **paineis L1 independentes** (`.panel`), largura do conteudo (nao 680px -- o `.oldsite-frame` ja limita), na ordem: (a) cabecalho do grupo; (b) **acoes do grupo**; (c) proxima partida (novo -- ver Fase 8); (d) metricas; (e) membros. `event-detail.css .detail-card` continua existindo para a tela de partida (`/futsal/{id}`) ate a Fase 2 do C36-B; **nao** editar la, criar as classes novas no `Detail.razor.css`.
2. (a) Cabecalho: `btn-ghost` "Voltar" (para `/grupos`) a esquerda; badge de esporte `--accent-soft` (nao verde); nome `--fs-2xl` (criar em tokens.css: 1.5rem; junto `--fs-xl` 1.25rem); cidade `--text-2`; a direita, para admin, `btn-ghost btn-sm` "Configuracoes" (engrenagem). Sem centralizar tudo -- alinhamento a esquerda como os paineis da home.
3. (b) Painel "Acoes" = a **navegacao interna do grupo**, visivel a todo membro (C36-C Fase 3 ja pede "Pagamentos" para todos): 3 a 4 botoes grandes em grid `repeat(auto-fit, minmax(160px, 1fr))`, cada um L2 (`--surface-2`, icone em `--accent-text`, rotulo, e uma linha `--text-2` de contexto: "Partidas -- 1 proxima", "Pagamentos -- 2 pendentes", "Ranking", "Configuracoes" (admin)). Hover L3 + `--accent-border`. E aqui que o Robson "nao acha os botoes": eles viram o painel mais visivel da tela. Convite (link + codigo + Copiar + WhatsApp) fica **dentro de Configuracoes para admin** e como `btn-secondary` "Convidar" nas acoes -- nao mais um bloco permanente no topo do hub.
4. (d) Metricas (`GroupMetrics.razor.css`): 4 cards L2 no mesmo grid; icone em `--accent-soft` circulo (**sem gradiente colorido por metrica**: azul/roxo/verde/laranja saem), numero `--text` `--fs-2xl` (criar em tokens.css: 1.5rem; junto `--fs-xl` 1.25rem), rotulo `--text-2`; "Atencao" em `--warning` so quando a taxa esta abaixo do limiar ja calculado no componente.
5. (e) Membros (`MembersManager`): painel L1 com `.panel-head` "Membros (4)"; linhas L2 com avatar/iniciais, nome, badges `Admin` (`--accent-soft`) e `Voce` (`--surface-3` + `--text-2`); acoes de admin (promover/remover) em menu `...` `btn-ghost btn-sm` a direita, **nao** botoes soltos. Pedidos pendentes de entrada como sub-lista no topo com "Aprovar" `btn-secondary btn-sm` e "Recusar" `btn-ghost btn-sm` -- "Aprovar todos" `btn-primary` **so** se houver >= 2 pedidos (senao nao e o proximo passo).
6. Nao-membro (linha ~214 do `Detail.razor`): um so painel L1 com `.empty-state` "Voce nao faz parte deste grupo" + `btn-primary` "Pedir para entrar" (ou "Cancelar pedido" ghost) -- nada de metricas nem membros.
Teste: `GroupDetailIntegrationTests` -- admin ve 4 acoes (Partidas, Pagamentos, Ranking, Configuracoes); membro ve 3 (sem Configuracoes) e nao ve link de convite; nao-membro ve so o `empty-state`. Contagens "N proxima(s)"/"N pendente(s)" batem com os dados seedados.

**Resultado**: `.detail-card` unico morreu no hub -- a pagina e uma coluna de `.panel` L1 (`.hub-stack`) em `Detail.razor.css`. (a) Cabecalho: Voltar ghost a esquerda, badge de esporte `--accent-soft`, titulo `--fs-2xl` (token novo com `--fs-xl`), cidade `--text-2`, engrenagem ghost a direita (admin). (b) Painel de acoes: grid `minmax(160px,1fr)` de itens L2 com icone `--accent-soft`, sub-linha `--text-2` ("N proximos" via `GroupDetailData.UpcomingEventsCount` novo -- count no service; "N pendente(s)"/"Em dia"), Convidar admin dispara WhatsApp. (c) Proxima partida fica para a Fase 8. (d) Metricas: 4 cards L2, icone `--accent-soft` circulo (gradientes coloridos mortos), valor `--fs-2xl`. (e) Membros: `GroupDetailMembers` reescrito com linhas L2, avatar/iniciais, badges `--accent-soft`/`--surface-3`; pedidos pendentes viraram sub-lista no topo do painel (Aprovar `btn-secondary btn-sm`, Recusar `btn-ghost btn-sm`, "Aprovar todos" `btn-primary` so com >= 2, usando `ApproveAllPendingAsync` ja existente) -- `GroupDetailPendingRequests` (checkboxes/sort) apagado junto com 268 linhas de CSS. Nao-membro: um painel `.empty-state` com "Pedir para entrar"/"Cancelar pedido"/link de login -- **membros, metricas e convite nao vazam mais para nao-membro** (mudanca de visibilidade, defensavel: a lista de membros era publica por acidente). Convite completo virou `Shared/Components/Groups/GroupInviteBlock` e mora em Configuracoes (`Features.razor`). `MembersManager` (config): acoes de admin em menu `...` (`btn-ghost btn-sm` + popover L3), linhas L2, sem botoes soltos. Codigo morto removido de `Detail.razor.cs` (sort/selecao/input de codigo sem uso). 11 chaves i18n novas x3 (acoes, subs, Aprovar todos, login, ver todos). Teste: `GroupDetailIntegrationTests` (3) -- admin ve nav completa + convidar sem invite-block; membro ve nav sem config/convite; nao-membro ve so empty-state (sem member-row/metric-card/hub-action).

**Metricas CSS (arquivos tocados, antes->depois)**: linhas 2548->2298 (-250); `linear-gradient` 19->12 (-7); vars legadas 93->39 (-54); `--ci-*` 180->134 (-46). `Detail.razor.css`, `MembersManager.razor.css`, `GroupMetrics.razor.css`, `GroupDetailMembers.razor.css`: 0 gradiente, 0 var legada, 0 `--ci-*`. Camadas: L0 fundo -> paineis `.panel` L1 -> itens L2 (acoes, membros, pedidos, metricas) -> hover/popover L3 (menu `...`).

#### Fase 8 -- Fluxo grupo -> membros -> partidas (ponto 8: telas "cruas", responsabilidades) -- EXECUTADO
Decisao (Robson + Senior): **nao reunificar**. Elas ja foram uma tela e foram separadas por peso; o problema e que separaram sem dar identidade. Papel de cada uma (vai para `docs/design-system.md` §6):
- `/grupo/{id}` **hub**: quem e o grupo (cabecalho), para onde ir (acoes), a proxima partida em 1 card, saude (metricas), quem esta (membros). E a "capa".
- `/grupo/{id}/partidas` **agenda**: lista completa proximas/realizadas, criar/editar, semanais. So partidas.
- `/grupo/{id}/pagamentos` **caixa**: C36-C Fase 3.
- `/grupo/{id}/ranking` **desempenho**; `/grupo/{id}/configuracoes` **admin** (features, Pix do grupo, convite).
Fazer:
1. **Sub-nav do grupo** compartilhada: componente `Shared/Components/Groups/GroupSubNav.razor` (abas `Visao geral · Partidas · Pagamentos · Ranking · [Configuracoes]`, `.seg-toggle` grande com `active` pela rota atual via `NavigationManager.Uri`), renderizado logo abaixo do cabecalho do grupo em **todas** as 5 telas. Com ela, "Voltar"/"Grupos" nas telas filhas viram redundantes -> **remover** dos filhos; manter so o breadcrumb `Grupos > Racha X > Partidas` (`.breadcrumb-nav`, ja em `shell.css`, L0). O painel de acoes do hub (Fase 7 item 3) **continua** -- e a versao "com contexto" (contadores) da mesma navegacao; a sub-nav e a versao compacta para quem ja esta dentro.
2. Cabecalho do grupo vira componente `GroupHeader.razor` (badge esporte, nome, cidade, sub-nav) usado pelas 5 telas -- hoje cada uma redesenha o proprio (`detail-header` em `Detail`, `Partidas`, `Ranking`, `Payments`). Um lugar, um visual.
3. `/partidas`: painel L1 "Partidas" com `.panel-head` (toggle Proximas/Realizadas a esquerda = `.seg-toggle`; "Semanais" `btn-secondary` + "Nova partida" `btn-primary` a direita, admin); tabela `GroupDetailEvents` com linhas L2 (`--surface-2`, hover L3), colunas `# · Quando (Fase 2) · Jogadores · Status · [Acessar]`; status como badges do vocabulario (`aberto` `--success-soft`, `proxima` `--accent-soft`, `cheia` `--warning-soft`, `cancelada` `--danger-soft`) -- hoje sao verde/amarelo legados. Empty state (Fase 6 do C36-B item 1): "Criar a primeira partida" `btn-primary` para admin; "O organizador ainda nao marcou a proxima partida" para jogador.
4. Hub, card "Proxima partida" (Fase 7 item c): reusar `ConfirmationCard`/`.home-event` da home (mover `.home-event*` para `components.css` como `.event-row*`) com o CTA do jogador ("Confirmar presenca" / "Pagar" / badge "Confirmado") -- e o **primario do jogador** no hub. Se nao ha proxima partida: `empty-state` com "Nova partida" (admin) ou frase (jogador). Dados: `Detail.razor.cs` ja carrega `upcoming` para a metrica "1 proximos" -- reaproveitar, sem query nova.
Teste: `GroupSubNavTests` (integracao, HTML das 5 rotas): aba ativa por rota; "Configuracoes" so para admin. Integracao: as 5 rotas renderizam `GroupHeader` com o nome do grupo; `/partidas` nao contem mais "Voltar".

**Resultado**: `GroupSubNav.razor` (`.seg-toggle` + `.seg-item`, aba ativa por `NavigationManager.Uri` -- `ToBaseRelativePath` nao tem barra inicial, normalizado com `"/" +`; Configuracoes so com `IsAdmin`, Ranking so com `EnablePostMatchRanking`) e `GroupHeader.razor` (badge esporte `--accent-soft`, nome `--fs-2xl`, cidade, sub-nav) novos em `Shared/Components/Groups/`; as 5 telas (`Detail`, `Partidas`, `Payments` nos dois ramos admin/membro, `Ranking`, `Features`) renderizam `<GroupHeader>` dentro de `.hub-stack` -- headers locais `detail-header` removidos junto com o Voltar/Grupos redundante. `Breadcrumb` agora resolve `Grupos > {nome do grupo} > ...` em rotas `/grupo/{id}` (uma query PK pequena, so quando o primeiro segmento e `grupo`) com `Hrefs` alinhados; `Breadcrumb.Group(s)` x3 novas. `.seg-toggle`, `.seg-item(.--active)`, `.seg-count`, `.event-row*`, `.status-badge(--confirmed/--pending)`, `.empty-state`, `.hub-stack` e `.panel*` foram para `components.css` (compartilhados; scoped CSS do Detail nao alcancava as outras telas). `/partidas`: painel L1 com `.panel-head--spread` (toggle `.seg-toggle` com contadores a esquerda; Semanais `btn-secondary` + Nova partida `btn-primary` a direita, admin); `GroupDetailEvents` ganhou scoped css proprio -- linhas L2 via `border-spacing`, hover `--accent-border`, badges `.et-status` no vocabulario novo (`aberto` success, `proxima` accent, `cheia` warning, `cancelada` danger) substituindo `player-tag`/`event-next-badge` legados e os textos PT hardcoded viraram chaves x3; empty state com "Criar a primeira partida" `btn-primary` (admin) / "O organizador ainda nao marcou a proxima partida" (jogador). Hub: painel "Proxima partida" com `.event-row` + CTA do jogador (Pagar `btn-primary` / "Comprovante em analise" `status-badge--pending` / "Confirmado" `status-badge--confirmed` / "Confirmar presenca" `btn-primary`) -- mesma regra de `OwesPayment`/`ProofSent` do `ConfirmationCard`; `GroupDetailData.NextEvent` novo com `Include(Confirmations)`, uma query a mais substituindo o count (o `UpcomingEventsCount` continua para a sub-linha da acao). `ConfirmationCard` nao foi reusado direto: ele exige uma `EventConfirmation` e nao cobre o caso "sem confirmacao" (Confirmar presenca). CSS morto: `hub-header-*`/`hub-sport-badge`/`hub-title`/`hub-location` (Detail, substituidos por `group-header-*`), `.partidas-title`, `.detail-title.ranking-title`. Teste: `GroupSubNavTests` (8) -- 5 rotas x aba ativa + `group-header` com o nome; Config so admin; sem "Voltar" em `/partidas`; breadcrumb com o nome do grupo. Camadas: L0 fundo/breadcrumb -> L1 `GroupHeader` + `.panel` por secao -> L2 `.seg-toggle`, linhas da tabela, `.event-row`, acoes `hub-action` -> L3 hover/focus (`--surface-3`, `--accent-border`). Metricas CSS tocadas: `Detail.razor.css` 316->228, `Partidas.razor.css` 13->9, `Ranking.razor.css` 22->15, `components.css` 131->362 (+`.seg-toggle`/`.event-row`/`.panel`/`.status-badge`/`.empty-state`/`.hub-stack`), `GroupHeader.razor.css` 43 novo, `GroupDetailEvents.razor.css` 120 novo; 0 gradientes novos, 0 `--ci-*` novo. 2520->2528 testes.

#### Ordem, PRs e evidencias
Uma PR por fase, nesta ordem: **1 (botoes + components.css) -> 7 (hub) -> 8 (sub-nav + partidas) -> 4 (/grupos) -> 2 (hover data) -> 5 (mailbox) -> 6 (profile) -> 3 (hero azul, ultima: e a que o Robson mais vai querer ajustar, melhor com tudo estavel)**. A Fase 1 e pre-requisito de todas. C36-C (nav sem Pagamentos, membro em `/pagamentos`) pode intercalar apos a 7.
Em cada PR, alem das metricas do C36-B (linhas, gradientes, vars legadas antes/depois; prints 1366/390; contraste verde): (i) diff do `docs/design-system.md` quando a fase cria regra; (ii) `MigratedFiles` cresce e o teste passa; (iii) **um paragrafo "camadas"** listando, para a tela, o que e L0/L1/L2/L3 -- se o Pleno nao consegue escrever esse paragrafo, o layout ainda esta plano.

#### O que NAO fazer
Nao tocar em regra de negocio, autorizacao (fora do que o C36-C ja pede) nem rotas -- so quando a fase diz "sem query nova", e literal. Nao mexer em `Pages/Futsal/*`, `Pages/Poker/*`, `Payment*` (C36-B Fases 2 e 3, depois). Nao criar 5a variante de botao; se parecer necessario, e um `btn-secondary` com icone. Nao inventar tom: se a tela pede um 4o nivel, reestruturar. Nao usar `--ci-*` em arquivo tocado. Nao "pular" a Fase 1 para ir direto no hub: sem `components.css` cada tela vai reinventar botao de novo e o Senior vai devolver o PR.
...
#### Review Senior da ressalva 1 do C36-D (PR #125, mergeada na `main`) -- APROVADO

Verificado localmente (`main` em `816d4d1`): build 0/0; 2567 testes (2543 verdes + 24 `ProgramConfigurationTests` ambientais). Regra implementada como proposta e um pouco melhor: bloqueio cobre divida de partida passada, comprovante em analise e futura nao-paga (`Pending`, precificada, ativa, nao-goleiro); ao sair sem divida, confirmacoes nao-pagas futuras passam pelo mesmo `CancelConfirmationAsync` do auto-cancelamento (promocao da lista de espera incluida) e a waitlist do grupo e limpa; confirmacoes pagas/gateway sao preservadas -- decisao correta (registro financeiro + vaga comprada). Audit com contadores. 8 testes de integracao nos dois caminhos. i18n x3.

Correcao da minha review: nao existe remocao de membro pelo admin no produto (`MembersManager` so promove/rebaixa), entao a parte "vale tambem pra remocao pelo admin" nao se aplica hoje; se um dia existir, deve reutilizar esta mesma regra.

Ressalva menor (nao vira ciclo): os cancelamentos gravam em contexto proprio antes do `SaveChanges` da membership -- se este falhar, as confirmacoes ja foram canceladas e o membro fica. Aceitavel (falha rara, estado recuperavel pelo proprio jogador), mas e o padrao a evitar em fluxo financeiro.
...
- **Desvio de processo (registrado, corrigido)**: durante a execucao cheguei a pushar 6 branches intermediarias (`feat/ciclo36d-fase*`) por ter lido "uma PR por fase" na secao de metricas acima; as regras 9/10 pedem 1 branch + 1 PR por ciclo. O Robson decidiu o PR unico; a branch consolidada `feat/ciclo36d` contem o mesmo topo. **As 6 branches intermediarias seguem no remoto, redundantes -- aguardando decisao para deletar.**
- **Numeros (regra 28)**: `dotnet build --no-incremental` -> **0 Warning(s) / 0 Error(s)**; `dotnet test` -> **Failed: 0, Passed: 2559, Skipped: 0, Total: 2559** (inicio do ciclo: 2513 -> +46). Sem falhas ambientais nesta maquina.
- **Funcionalidade nova alem do desenho** (Fase 4, documentada na fase): `LeaveGroupAsync` (admin unico bloqueado + auditoria `GroupMemberRemoved`), `GetMyPendingRequestsAsync` (pedidos do proprio usuario nao apareciam em lugar nenhum), `CancelJoinRequestAsync` passou a revalidar ownership. Authz apenas reforcada; nenhuma regra financeira tocada.
- **Desvio tecnico na Fase 3**: degrade termina em `--accent` (nao `--accent-light` -- #3b82f6 reprovaria AA ~3.4:1); `--accent-fg-2` e hex solido #eef4ff (rgba .82 sugerido daria ~4.0:1). 5 pares on-accent testados em `DesignTokensContrastTests`.
- **Novo guardiao**: `CssNoLegacyVarsTests` com `MigratedFiles` (32 arquivos) -- falha em `linear-gradient` fora de `.home-hero*` e em qualquer `--ci-*`/`--sport-*` nos migrados.
- **Screenshots 1366/390**: nao produzidos -- E2E/browser nunca executado neste ambiente; validacao visual fica com o Robson (divida conhecida, regra 26 registrada como pendencia).
- **Fora de escopo** (nao migrados): `Pages/Groups/Create`, gestao de pagamentos do grupo (`Payments.razor.css`, `GroupDetailPaymentsModal`), `Pages/Futsal/*`, `Pages/Poker/*`, `Payment*` (C36-B fases 2-3), Identity/Admin.
...
### C36-D -- achados durante a execucao (todos corrigidos no proprio ciclo, registrados para memoria)

- **`GroupDetailEvents` exibia `StartsAt` em UTC cru** -- o resto do app converte com `.ToLocalTime()`, a tabela de partidas nao. Corrigido na Fase 2 junto ao tooltip.
- **`mk-input` era scoped a `.marketplace-page`** -- os campos do mailbox e do profile usavam a classe mas **nunca foram estilizados de verdade** (CSS isolado nao alcançava). Substituído por `.input` compartilhado na Fase 5. Mesma classe de problema para `mk-btn`/`mk-empty`/`market-empty`.
- **`GroupSubNav` (Fase 8) tinha matching de rota quebrado** -- `ToBaseRelativePath` retorna sem `/` inicial; corrigido antes do commit.
- **`RoleClass` nas bolhas do `MailboxThreadPane` era propriedade morta** -- nunca setada; removida.
- **`AvatarUploadSection` era componente morto** (so auto-referencia) -- apagado na Fase 6.
- **CSS morto acumulado**: `.group-card*`/`.groups-*` inteiro sem uso apos a Fase 4 (`event-listing.css` 322->76, `events.css` 1551->1218); `onboarding-hint` duplicado no `Index.razor.css`; `detail-header`/`detail-actions` apos a Fase 8.
- **Conflito de spec**: a secao de metricas do C36-D pedia "uma PR por fase", as regras 9/10 pedem 1 branch + 1 PR por ciclo. Resolvido pelo Robson: PR unico na branch `feat/ciclo36d` (commits separados por fase). **Sugiro alinhar o texto do plano para nao repetir o desvio.**
